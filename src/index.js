...
const optionsSchema = {
  ...
  description:
    "The configuration for an instance of the Eloquent Git version Webpack plugin",
  type: "object",
  additionalProperties: false,
  properties: {
    path: {
      description:
        "The path to use for the version file, relative to the output path",
      type: "string",
      minLength: 1,
      default: "VERSION",
    },
    name: {
      description:
        "The name to use for the version variable, accessible on the window object",
      type: "string",
      minLength: 1,
      default: "VERSION",
    },
...
module.exports = function GitVersionWebpackPlugin(options = {}) {
  ...
  async function version() {
    ...
    try {
      const { stdout: describe } = await execFile("git", [
        "describe",
        "--long",
        "--tags",
      ]);
  ...
  async function handleAlterAssetTags(data) {
    ...
    scripts.unshift({
      ...
      innerHTML: `window[${JSON.stringify(versionName)}] = ${JSON.stringify(
        v
      )}`,
    });
